...
class AuthService {
    ...
    async loginAuthenticatorHandler(data: LoginData):Promise<User> {
    ...
    async login ({request}):Promise<Response> {
        ...
        // commit the session
        const headers = await this.setSession(request, user);
